...
const SchedulesRestHandler = {
  async createSchedule(
    ...
    try {
      const { schedule_name, time_in, time_out } = req.body;
  ...
  async deleteSchedule(
    ...
    try {
      ...
      const deletedSchedule = await SchedulesController.deleteSchedule(
        Number(schedule_id)
      );
  ...
  async readSchedule(
    ...
    try {
      ...
      const schedule = await SchedulesController.readSchedule(
        Number(schedule_id)
      );
  ...
  async assignSchedule(
    ...
    try {
      ...
      /*
          ...
          */
      const employeeSchedules = await SchedulesController.getEmployeeSchedule({
        employee_id,
      });
      ...
      if (employeeSchedules !== null) {
        employeeSchedules.forEach((employeeSchedule) => {
          const workdaysArray = JSON.parse(
            (employeeSchedule.dataValues as unknown as EmployeeSchedule)
              .workdays
          );
  ...
  async getEmployeeSchedule(
    ...
    try {
      const { employee_id, schedule_id } = req.query;
  ...
  async getEmployeeScheduleById(
    ...
    try {
      const { employee_schedule_id } = req.params;
      ...
      const employeeSchedule =
        await SchedulesController.getAssignedScheduleById(
  ...
  async deleteEmployeeScheduleById(
    req: Request,
    res: Response,
    next: NextFunction
  ): Promise<void> {
    try {
      ...
      const deletedEmployeeSchedule =
        await SchedulesController.deleteAssignedScheduleById(
          Number(employee_schedule_id)
        );
  ...
  async updateEmployeeScheduleById(
    req: Request,
    res: Response,
    next: NextFunction
  ): Promise<void> {
    try {
      ...
      const employeeSchedules = await SchedulesController.getEmployeeSchedule({
        employee_id,
        employee_schedule_id: Number(employee_schedule_id),
      });
      ...
      const updatedEmployeeSchedule =
        await SchedulesController.updateEmployeeSchedule(
          {
            EmployeeID: employee_id,
            ScheduleID: schedule_id,
            Workdays: JSON.stringify(workdays),
  ...
  async getEmployeeWeeklySchedule(
    req: Request,
    res: Response,
    next: NextFunction
  ): Promise<void> {
    try {
      ...
      if (employeeSchedule.length === 0) {
        ...
        /*
                ...
                2. Sort workdays
                */
        const weekSchedule = employeeSchedule
          ?.map((item) => {
            const weekWorkdays = getWeekDates().filter((weekdates) =>
              item.workdays.includes(weekdates)
            );
  ...
  async getEmployeeMonthlySchedule(
    req: Request,
    res: Response,
    next: NextFunction
  ): Promise<void> {
    try {
      ...
      if (employeeSchedule.length === 0) {
        ...
        /*
                ...
                2. Sort workdays
                */
        const monthSchedule = employeeSchedule
          ?.map((item) => {
            const monthWorkdays = getMonthDates().filter((monthDates) =>
              item.workdays.includes(monthDates)
            );
