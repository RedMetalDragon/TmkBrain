...
export async function getTimeInAndOut(
  date: string,
  employee_id: number
): Promise<Record<string, any>> {
  // Get earliest and latest log for the day
  const timeInOut = await SchedulesController.getTimeInAndOut(
    date,
    employee_id
  );
...
export async function getTimeInAndOutNightShift(
  yesterday: string,
  currentDate: string,
  employee_id: number
): Promise<Record<string, any>> {
  // Get earliest log from yesterday
  const timeInOutYesterday = await SchedulesController.getTimeInAndOut(
    yesterday,
    employee_id
  );

  // Get latest log today
  const timeInOutToday = await SchedulesController.getTimeInAndOut(
    currentDate,
    employee_id
  );
