...
import {PUNCH_IN_ACTION, PUNCH_OUT_ACTION} from "../constants";
...
const AttendanceService = {
    ...
    async getlastActionMade(employeeId: number): Promise<string | Error> {
        try {
            const activeRecord = await Attendance.findOne({
                where: {
                    EmployeeID: employeeId,
            ...
            if (activeRecord !== null) {
                return PUNCH_IN_ACTION;
            } else {
                return PUNCH_OUT_ACTION
            }
        } catch (error) {
            console.log(error);
            return error as Error;
